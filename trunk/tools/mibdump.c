 ...
 * mibdump.c --
 *
 *      Dump a MIB contents conforming to a given format.
 *
...
static MibdumpDriver *firstDriver;
static MibdumpDriver *lastDriver;
static MibdumpDriver *defaultDriver;
static char *output;
...
void *xmalloc(size_t size)
{
    char *m = malloc(size);
    if (! m) {
	fprintf(stderr, "mibdump: malloc failed - running out of memory\n");
	exit(1);
    }
    return m;
}
...
void *xrealloc(void *ptr, size_t size)
{
    ...
    if (! m) {
	fprintf(stderr, "mibdump: realloc failed - running out of memory\n");
	exit(1);
    }
    return m;
}
...
char *xstrdup(const char *s)
{
    char *m = strdup(s);
    if (! m) {
	fprintf(stderr, "mibdump: strdup failed - running out of memory\n");
	exit(1);
    }
    return m;
}
...
void mibdumpRegisterDriver(MibdumpDriver *driver)
{
    int i;
...
static void formats()
{
    MibdumpDriver *driver = firstDriver;
...
{
    int i;
    MibdumpDriver *driver;
    char *value = NULL;
    
    fprintf(stderr,
	    "Usage: mibdump [options] [module or path ...]\n"
...
static void version() { printf("mibdump " SMI_VERSION_STRING "\n"); exit(0); }
static void config(char *filename) { smiReadConfig(filename, "mibdump"); }
static void level(int lev) { smiSetErrorLevel(lev); }
static void silent() { flags |= MIBDUMP_FLAG_SILENT; }
static void preload(char *module) { smiLoadModule(module); }
static void unified() { flags |= MIBDUMP_FLAG_UNITE; }
...
static void format(char *form)
{
    ...
    if (!driver) {
	fprintf(stderr, "mibdump: invalid dump format `%s'"
...
int main(int argc, char *argv[])
{
    ...
    if (!driver) {
	fprintf(stderr, "mibdump: no dump formats registered\n");
	smiExit();
	exit(1);
    }
    ...
    if (flags & MIBDUMP_FLAG_UNITE && driver->ignflags & MIBDUMP_DRIVER_CANT_UNITE) {
	fprintf(stderr, "mibdump: %s format does not support united output:"
		" ignoring -u\n", driver->name);
	flags = (flags & ~MIBDUMP_FLAG_UNITE);
    }

    if (output && driver->ignflags & MIBDUMP_DRIVER_CANT_OUTPUT) {
	fprintf(stderr, "mibdump: %s format does not support output option:"
		" ignoring -o %s\n", driver->name, output);
	output = NULL;
    }
	...
	if (smiModule) {
		...
		if (! (flags & MIBDUMP_FLAG_SILENT)) {
		    fprintf(stderr,
			    "mibdump: module `%s' contains errors, "
	    ...
	    fprintf(stderr, "mibdump: cannot locate module `%s'\n",
		    argv[i]);
	}
    }
