 ...
 *
 * @(#) $Id: dump-tree.c,v 1.13 2000/05/02 12:57:17 strauss Exp $
 */
...
static void addModule(char *name)
{
    Module *mPtr;
...
static int pruneSubTree(SmiNode *smiNode)
{
	...
	if (strcmp(mPtr->moduleName, smiGetNodeModule(smiNode)->name) == 0) {
	    return 0;
	}
    }
...
int dumpTree(char *modulename, int flags)
{
	...
	if (! (flags & SMIDUMP_FLAG_SILENT)) {
	    printf("# %sregistration tree (generated by smidump "
		   VERSION ")\n\n", unite ? "united " : modulename);
	}
